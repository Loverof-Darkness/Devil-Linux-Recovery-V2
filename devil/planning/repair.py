...
def build_repair_plan(target: TargetLayout) -> RepairPlan:
    ...
    steps = (
        ...
        RepairStep(
            ...
            "Mount the resolved root, separate /boot when explicitly identified, and the selected EFI System Partition for the transaction.",
            "mount_target",
            "MEDIUM",
            reversible=True,
        ),
        RepairStep(
            "preflight",
            "Validate the mounted repair target",
            "Confirm the selected root is a Linux installation, the selected EFI device is mounted at /boot/efi, the exact Btrfs subvolume is honored, and the target GRUB tooling exists.",
            "preflight_target",
            "MEDIUM",
            reversible=True,
        ),
        RepairStep(
            ...
            "Create a timestamped backup of the target EFI loader directory and preserve existing EFI files before bootloader mutation.",
            "backup_efi",
            "MEDIUM",
            reversible=True,
        ),
        ...
        RepairStep(
            ...
            "Generate a fresh GRUB configuration using the repaired Linux installation's own userspace tools before changing firmware BootOrder.",
